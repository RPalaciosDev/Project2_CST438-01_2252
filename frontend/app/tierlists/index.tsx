...
export default function TierList() {
  ...
  const fetchTemplateData = async (id: string) => {
    try {
      ...
      console.log(
        'Template imageIds check:',
        response.data.imageIds
          ? `Has ${response.data.imageIds.length} imageIds`
          : 'No imageIds field found',
      );
      console.log(
        'Template images check:',
        response.data.images
          ? `Has ${response.data.images.length} images`
  ...
  const renderDraggableItem = (item: TierItem, index: number) => {
    if (isWeb) {
      return (
        ...
        >
          <Image source={item.image} style={{ width: 80, height: 80, borderRadius: 5 }} />
        </div>
      );
    } else {
      return (
          ...
          <View style={[customStyles.tierContent, { flexDirection: 'row', flexWrap: 'wrap' }]}>
            {tierItems
              ...
              .map((item) => (
                ...
                >
                  <Image source={item.image} style={{ width: 60, height: 60, borderRadius: 4 }} />
                  {isWeb && (
                    <div
      ...
      try {
        ...
        if (templateId) {
          const templateResponse = await axios.get(
            `${TIERLIST_API_URL}/api/templates/${templateId}`,
          );
          ...
          if (templateData && templateData.tags && templateData.tags.length > 0) {
            ...
            await axios.post(
              `${AUTH_SERVICE_URL}/api/user/tags/record/${user.id}`,
              {
      ...
      if (isDailyTierlist) {
        try {
          ...
          if (!completionResponse || !completionResponse.success) {
            ...
            Alert.alert(
              'Warning',
              "Your rankings were saved, but we couldn't mark the daily tierlist as completed. You may be able to complete it again.",
            );
          } else {
  ...
  return (
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={[styles.container, customStyles.mainContainer]} ref={containerRef}>
        {/* Back button */}
        <TouchableOpacity style={customStyles.backButton} onPress={handleBackPress}>
          ...
          <View style={customStyles.templateHeader}>
            ...
            <View style={customStyles.templateStats}>
              ...
              {template.tags && template.tags.length > 0 && (
                <Text style={customStyles.statText}>Tags: {template.tags.join(', ')}</Text>
              )}
            </View>
          </View>
        )}
          ...
          <View style={customStyles.dialogOverlay}>
            <View style={customStyles.dialogContainer}>
              ...
              <View style={customStyles.dialogButtonContainer}>
                ...
                <TouchableOpacity
                  style={customStyles.dialogConfirmButton}
        ...
        {showMatchButton && (
          ...
          >
            <TouchableOpacity
              style={[customStyles.matchButton, isSubmitting && customStyles.matchButtonDisabled]}
