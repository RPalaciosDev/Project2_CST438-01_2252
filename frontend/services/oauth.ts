...
const getRedirectUri = () => {
  ...
  if (Platform.OS === 'web' && process.env.NODE_ENV === 'production') {
    ...
    const productionRedirectUri =
      'https://frontend-production-c2bc.up.railway.app/auth/google/callback';
    console.log('Production Redirect URI for Google OAuth:', productionRedirectUri);
    console.log(
      'Make sure this is added to Google Cloud Console > Credentials > Authorized redirect URIs',
    );
...
const fetchAccessTokenFromCode = async (code: string, redirectUri: string) => {
  try {
    const CLIENT_ID =
      ...
      '90481875753-p89h3cguug4634l6qj5jbe5ei11omguo.apps.googleusercontent.com';
    const CLIENT_SECRET =
      Constants.expoConfig?.extra?.googleClientSecret || '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';
    ...
    if (tokenData.error) {
      throw new Error(
        `Google OAuth error: ${tokenData.error} - ${tokenData.error_description || 'No description'}`,
      );
    }
...
const exchangeGoogleTokenForJWT = async (token: string, API_URL: string) => {
  try {
    ...
    console.log(
      'Full user data structure received from backend:',
  ...
  try {
    ...
    if (result.type === 'success') {
      ...
      if (result.params && result.params.code) {
        ...
        console.log(
          'ID token first few chars:',
          tokenData.idToken ? tokenData.idToken.substring(0, 10) + '...' : 'N/A',
        );
