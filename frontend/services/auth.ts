...
const API_URL = (() => {
  ...
  return Platform.OS === 'web' ? 'http://localhost:8081' : 'http://10.0.2.2:8081'; // Use 10.0.2.2 for Android emulator or your computer's actual IP address
})();
...
registrationAxios.interceptors.response.use(null, async (error) => {
  ...
  if (shouldRetry && error.config && error.config.__retryCount < 3) {
    // More retries for registration
    error.config.__retryCount = error.config.__retryCount || 0;
    error.config.__retryCount += 1;
...
registrationAxios.interceptors.request.use(
  async (config) => {
    try {
      const token = await storage.getItem('token');
      if (token) {
  ...
  try {
    ...
    if (result.type === 'success') {
      ...
      if (result.params && result.params.code) {
        ...
        console.log(
          'Using ID token for backend authentication - length:',
          tokenData.idToken.length,
          'preview:',
          tokenData.idToken.substring(0, 10) + '...',
        );
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (
        ...
        console.log(
          'User has all required fields but hasCompletedOnboarding is not set. Setting it now...',
        );
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  register: async (username: string, email: string, password: string) => {
    try {
      ...
      const signupResponse = await registrationAxios.post(
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.code === 'ECONNABORTED') {
          set({
            error:
              'Registration timed out. Our servers might be experiencing high load. Please try again later.',
            isLoading: false,
          });
          ...
          set({
            error:
              'Server error during registration. This might be due to an email or username already in use, or the server is experiencing issues.',
            isLoading: false,
          });
        } else {
          set({
            ...
            if (response.status === 200 && response.data) {
              ...
              console.log('checkStatus: Retrieved user data:', {
                ...
                hasCompletedOnboarding:
                  user.hasCompletedOnboarding === true ? 'true' : 'false/undefined',
  ...
  updateUserPreferences: async (lookingFor: string) => {
    try {
      ...
      console.log(
        `Updating user preferences to "${lookingFor}" at ${API_URL}/api/auth/update-profile`,
      );
  ...
  // Add method to update user's relationship type
  updateUserRelationshipType: async (relationshipType: string) => {
    try {
      set({ isLoading: true, error: null });
      ...
      console.log(
        `Updating user relationship type to "${relationshipType}" at ${API_URL}/api/auth/update-profile`,
      );
      ...
      if (response.status === 200) {
        // Update the user in state with the new relationship type
        const updatedUser = { ...user, relationshipType };
        ...
        throw new Error('Failed to update user relationship type');
      }
    } catch (error) {
      console.error('Update user relationship type error:', error);
      set({
        error: error instanceof Error ? error.message : 'Failed to update relationship type',
        isLoading: false,
      });
      return false;
    }
  ...
  updateUserPicture: async (pictureUrl: string) => {
    try {
      ...
      console.log(
        `Updating user profile picture to "${pictureUrl}" at ${API_URL}/api/auth/update-profile`,
      );
  ...
  markDailyTierlistCompleted: async () => {
    try {
      ...
      const response = await axiosInstance.post(
        `${TIERLIST_API_URL}/api/daily/complete`,
        {},
        {
          headers: {
            'X-User-ID': userId,
  ...
  setDailyTierlist: async (templateId: string) => {
    try {
      ...
      const response = await axiosInstance.post(
        `${TIERLIST_API_URL}/api/daily/${templateId}`,
        {},
        {
          headers: {
            'X-User-ID': userId,
  ...
  updateOnboardingStatus: async (completed: boolean) => {
    try {
      ...
      console.log(
        `Setting hasCompletedOnboarding to ${completed} at ${API_URL}/api/auth/update-profile`,
      );
